...
<c:if test="${!empty ciCnt}">
	<script>
		alert("커멘트가 정삭적으로 등록되었습니다.");
	</script>
</c:if>
<c:if test="${!empty ciDelCnt}">
	<script>
		alert("커멘트가 정삭적으로 삭제되었습니다.");
	</script>
</c:if>
<body>
		...
		<div class="col-sm-6 col-md-offset-3">
			...
			<div class="form-group">
				...
				<textarea class="form-control" cols="40" id="bitext" name="bitext"
					rows="10" disabled>${bi.bitext}</textarea>
			</div>

			<div class="form-group">
				<!-- Message field -->
				<label class="control-label " for="bifile">파일</label> <a
					href="${bi.bifile}">${bi.bifile}</a>
			</div>

			<div class="form-group">
				<button class="btn btn-primary " data-page="/board/boardList">리스트가기</button>
			</div>
